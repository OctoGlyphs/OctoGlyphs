...
export function shouldEmitModelEvents(config) {
    return isEnabled(config) && config?.emitModelEvents !== false;
}
export function shouldEmitToolEvents(config) {
    return isEnabled(config) && config?.emitToolEvents !== false;
}
export function createPromptSentEvent(event) {
    return compactEvent({
        type: "prompt.sent",
        timestamp: Date.now(),
...
export function createModelStartedEvent(event) {
    return {
...
export function createModelEndedEvent(event) {
    return createResponseCompletedEvent(event);
}
export function createAgentEndPromptSentEvent(_event) {
    return compactEvent({
        type: "prompt.sent",
        timestamp: Date.now(),
        source: "agent_end"
...
export function createTurnStartedPromptSentEvent(_event) {
    return compactEvent({
        type: "prompt.sent",
        timestamp: Date.now(),
...
export function createInboundMessagePromptSentEvent(_event) {
    return compactEvent({
        type: "prompt.sent",
        timestamp: Date.now(),
...
export function createInboundMessageResponseFallbackEvent(_event) {
    return compactEvent({
...
export function createAgentEndModelEndedEvent(event) {
    return compactEvent({
        ...createResponseCompletedEvent(event),
        source: "agent_end"
    });
}
export function createMessageSentModelEndedEvent(event) {
    return compactEvent({
        ...createResponseCompletedEvent(event),
        source: "message_sent"
    });
}
export function createToolUsedEvent(event) {
...
export function getCompanionGameUrl(config) {
    return config?.gameUrl ?? DEFAULT_GAME_URL;
}
export function getPublicCompanionGameUrl(config) {
...
export function getRunId(event) {
...
function createResponseCompletedEvent(event) {
    return compactEvent({
        type: "response.completed",
        timestamp: Date.now(),
        duration_ms: safeNonNegativeNumber(event.durationMs ?? event.duration_ms),
