...
- **What's next**: Commit and push this fix, then Ed pulls on the Mac, rebuilds/re-packs/reinstalls the OpenClaw package, restarts the gateway, hard-refreshes `/octoglyphs`, and verifies both fullscreen and smaller-panel layouts no longer show the small canvas or trait-panel sliver.
