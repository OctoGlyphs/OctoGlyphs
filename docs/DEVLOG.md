...
### Latest: Add Claude Code Launcher
- **Commit**: This commit.
...
- **What's next**: Ed pulls on Mac, runs the Claude Code source-install launcher flow, confirms `claude-octoglyphs` starts an interactive Claude Code session with OctoGlyphs attached, then opens the tank URL from `~/.octoglyphs-claude-code.json` and verifies prompt gems still spawn.
