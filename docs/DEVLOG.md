...
- **What's next**: Ed pulls on the Mac, rebuilds/re-packs/reinstalls the OpenClaw package, restarts the gateway, then resizes the `/octoglyphs` tab/panel to confirm the same amount of tank remains visible while the whole UI scales proportionally.
