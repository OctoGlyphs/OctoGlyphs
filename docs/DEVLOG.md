...
### Latest: Add OpenClaw Turn Fallback Hooks
- **Commit**: This commit.
...
- **What's next**: Pull and reinstall the packed OpenClaw plugin on Mac, then verify `curl -N http://localhost:18789/octoglyphs/stream` shows generic `prompt.sent`, `response.started`, and `response.completed` after a normal no-tool prompt and the tank spawns gems.
