...
### Latest: Add OpenClaw Plain-Chat Activity Fallback
- **Commit**: This commit.
- **Changed**: Added a privacy-preserving `agent_end` fallback to the OpenClaw adapter so normal chat turns can feed OctoGlyphs when sanitized `model_call_started`/`model_call_ended` hooks do not fire. The fallback emits only generic `prompt.sent` and `response.completed` events, dedupes by run id when model-call telemetry already covered the turn, and never reads final messages. Updated OpenClaw README, adapter notes, and the adapter contract test to cover the fallback and leak probes.
- **Why**: Mac testing showed `/octoglyphs/stream` stayed connected but normal no-tool prompts emitted nothing after the OpenClaw privacy hardening, while tool calls still emitted `tool.used`. We needed reliable prompt feeding without reverting to content-bearing prompt hooks.
...
- **What's next**: Pull and reinstall the packed OpenClaw plugin on Mac, then verify `curl -N http://localhost:18789/octoglyphs/stream` shows generic activity after a normal no-tool prompt and the tank spawns gems.
