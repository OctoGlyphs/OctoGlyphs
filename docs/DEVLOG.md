...
### Latest: Polish Claude Code Plugin Startup Flow
- **Commit**: Pending in this working tree.
- **Changed**: Synced the latest built tank bundle into the Claude Code plugin public directory, added companion stream status readouts in the tank UI, hardened music playback so missing bundled tracks are skipped instead of throwing cache errors, made the Claude Code `SessionStart` hook synchronous, and taught the hook to return a visible Claude Code `systemMessage` with the tank URL and privacy notice. The startup copy now says, "Your OctoGlyph is blindly feeding on this Claude Code session." The Claude Code hook test was expanded to verify streamed events can be received from the sidecar.
- **Why**: Claude Code users need the same obvious entry point that OpenClaw users get from `/octoglyphs`: start Claude Code, see the local tank URL, open the browser, and work normally while sanitized activity events feed the tank. The tank also needed visible connection diagnostics so prompt events can be debugged without opening browser internals.
- **Verification**: Ed manually rebuilt the game from `game`, copied `game/dist` into `plugin/hosts/claude-code/public`, launched Claude Code with `claude --plugin-dir .`, confirmed the startup message appears, and confirmed prompts spawn gems through the Claude Code hook/sidecar/browser path. Automated build/test commands were intentionally not run from Ember during this Ubuntu-safe pass.
- **What's next**: Test whether a visible but unfocused OctoGlyphs window continues live swimming instead of falling back to hidden-window ledger behavior; add focus/visibility/simulation diagnostics if needed.
